...
namespace FTPbox
{
    public partial class frmMain : Form
    {
        ...
        {
            Process.Start(@"https://sourceforge.net/donate/index.php?group_id=538656");
        }
        #endregion
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            string meh = "";
            foreach (string y in allfilesandfolders)
            {
                meh = meh + Environment.NewLine + y;
            }
            MessageBox.Show(meh);
        }
        ...
        List<string> allfilesandfolders = new List<string>();
        ...
        private void List_all()
        {
            ...
            foreach (FtpFileInfo fi in ftp.GetFiles())
            {
                ...
                allfilesandfolders.Add(fi.Name);
            }
            foreach (FtpDirectoryInfo dir in ftp.GetDirectories())
            {
                alldirs.Add(dir.Name);
                ftp.SetCurrentDirectory(noSlashes(rPath()) + @"/" + dir.Name);
                foreach (FtpDirectoryInfo dir2 in ftp.GetDirectories())
                {
                    alldirs.Add(dir2.Name);
                    ftp.SetCurrentDirectory(noSlashes(rPath()) + @"/" + dir.Name + @"/" + dir2.Name);
                    foreach (FtpDirectoryInfo dir3 in ftp.GetDirectories())
                    {
                        alldirs.Add(dir3.Name);
                        ftp.SetCurrentDirectory(noSlashes(rPath()) + @"/" + dir.Name + @"/" + dir2.Name + @"/" + dir3.Name);
                        foreach (FtpDirectoryInfo dir4 in ftp.GetDirectories())
                        {
                            alldirs.Add(dir4.Name);
                        }
                        ftp.SetCurrentDirectory(noSlashes(rPath()) + @"/" + dir.Name + @"/" + dir2.Name + @"/" + dir3.Name);
                        foreach (FtpFileInfo fi4 in ftp.GetFiles())
                        {
                            allfiles.Add(fi4.Name);
                            allfilesandfolders.Add(fi4.Name);
                        }
                    }
                    ftp.SetCurrentDirectory(noSlashes(rPath()) + @"/" + dir.Name + @"/" + dir2.Name);
                    foreach (FtpFileInfo fi3 in ftp.GetFiles())
                    {
                        allfiles.Add(fi3.Name);
                        allfilesandfolders.Add(fi3.Name);
                    }
                }
                ftp.SetCurrentDirectory(noSlashes(rPath()) + @"/" + dir.Name + @"/");
                foreach (FtpFileInfo fi2 in ftp.GetFiles())
                {
                    allfiles.Add(fi2.Name);
                    allfilesandfolders.Add(fi2.Name);
                }
            }
